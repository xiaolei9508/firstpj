...
import com.cmft.slas.cmuop.common.exception.BaseException;
 ...
 * @date 2018/05/08
 */
@Service("cmuopQuartzService")
public class CmuopQuartzServiceImpl implements CmuopQuartzService {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    @Autowired
    @Qualifier("cmuopScheduler")
        ...
        try {
            if (checkJobExists(jobName, jobGroup)) {
                ...
                logger.info("===> AddJob fail, job already exist, jobGroup:{}, jobName:{}, cover:{}", jobGroup,
                        jobName);
                throw new BaseException(String.format("Job已经存在, jobName:{%s},jobGroup:{%s}", jobName, jobGroup));
            }
    ...
    public void editJob(QuartzTaskInfoDTO info) {
        ...
        try {
            if (!checkJobExists(jobName, jobGroup)) {
                throw new BaseException(String.format("Job不存在, jobName:{%s},jobGroup:{%s}", jobName, jobGroup));
            }
            TriggerKey triggerKey = TriggerKey.triggerKey(jobName, jobGroup);
